...
- **Respaldo diario a OneDrive (el que de verdad sale de la PC)**: como cada marca corre en una PC de la agencia y no hay bucket S3 contratado, la copia que sobrevive a que se rompa el disco es la que sube a la carpeta de OneDrive de la empresa. Lo hace `scripts\windows\Respaldo-Calidad.ps1`, y lo **dispara el vigilante** (que ya corre cada 5 minutos en la sesión del usuario): **12:00**, con reintentos hasta las **19:00** si la PC estaba apagada, y sin repetir el del día si ya salió bien. **No hay que instalar ninguna tarea programada ni ser administrador** — antes sí, y eso es justo lo que nunca se hizo: Ford estuvo 19 días sin respaldo. El destino se detecta solo (la carpeta `OneDrive - <empresa>` de la sesión, subcarpeta `Respaldos Calidad`). Deja el resultado en `Respaldos\ultimo-respaldo.json`, que el compose monta en el backend en solo lectura: el Dashboard muestra la casilla **"Copia diaria a OneDrive"** en rojo si el último respaldo falló o quedó más viejo que **72 h**, y el vigilante manda un correo de aviso después de las 19:00. El paso a paso para la PC está en **[scripts/windows/README.md](scripts/windows/README.md)** (Paso 7).

Variables nuevas relevantes (ver `.env.prod.example`): `FRONTEND_URL`, `RATE_LIMIT_POR_MINUTO`, el bloque `BACKUP_*` / `BACKUP_S3_*`, y el bloque `RESPALDO_*` del respaldo a OneDrive (todas opcionales: `RESPALDO_DESTINO_NUBE`, `RESPALDO_RETENCION_NUBE`, `RESPALDO_SIN_CLAVE`, `RESPALDO_ALERTA_EMAIL`).
